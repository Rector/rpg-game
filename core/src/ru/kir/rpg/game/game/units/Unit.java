...
public abstract class Unit implements Poolable, MapElement {
    ...
    public boolean takeDamage(Unit unit, int amount) {
        stats.hp -= amount;
        ...
        if (stats.hp <= 0) {
            ...
            unit.stats.addExp(this.stats.maxHp);
        }
        return stats.hp <= 0;
    }
    ...
    public void goTo(int argCellX, int argCellY) {
        ...
        if (stats.movePoints > 0 && Math.abs(argCellX - cellX) + Math.abs(argCellY - cellY) == 1
                && (stats.movePoints - gc.getGameMap().getCellType(argCellX, argCellY).getComplexity()) >= 0) {
            stats.movePoints -= gc.getGameMap().getCellType(argCellX, argCellY).getComplexity();
            targetX = argCellX;
            targetY = argCellY;
    ...
    public boolean amIBlocked() {
        return !(gc.isCellEmpty(cellX - 1, cellY) || gc.isCellEmpty(cellX + 1, cellY) || gc.isCellEmpty(cellX, cellY - 1)
                || gc.isCellEmpty(cellX, cellY + 1));
    }
